var User = require('../nsa/db');                     // OUR DATABASE STUFF LIVES HERE
var fs = require("fs");
...
const mqttClient  = mqtt.connect(process.env.MQTTBOX, {
  clean: false,
  clientId: 'test'
});
...
mqttClient.on('connect', function () {
  mqttClient.subscribe('/cmd/add', {qos: 1});
...
var onStart = () => {
      ...
      if (u.src == 'discord') {
        ...
        if (u.links) {
          ...
          for (let link of tmplinks) {
            mqttClient.subscribe(link.remote, {qos: 1});
  ...
  let myVar = setInterval(function(){ heartbeat('/maclawran.slack.com/messages/CFMMBM5AP') }, 60000);
};
  ...
  /*  OLD VERSION GOT OUT OF CONTROL
    ...
    */
    ...
    if(message.type === 'message' && message.attachments && !message.text) {
      ...
      mqttClient.publish(link.channel, msg, {qos: 1});
    }
    ...
    if(message.type === 'message' && Boolean(message.text)) {
      ...
      switch(command) {
        ...
        case 'mirror':
          if (link.channel === link.remote) {
            // bot.postMessage(channel.id, "`um, you really don't want to link to yourself.`", params); 
            bot.postMessage(message.channel, "`um, you really don't want to link to yourself.`", params); 
            return;
          }
          if (!link.remote) {
            // bot.postMessage(channel.id, "`error: remote Slack channel not specified`", params); 
          ...
          console.log('---------------------------------');
          if (result.length > 0) {
            // bot.postMessage(channel.id, "`oops! you're already mirroring this channel to there`", params); 
          ...
          if (channel) {
            link.channelName=channel.channelName;
          } else {
          ...
          mqttClient.subscribe(link.channel, {qos: 1}); // SUBSCRIBE TO THE OTHER SIDE
          ...
          mqttClient.publish(tmplink.remote, msg, {qos: 1});
      ...
      if(usr && usr.name !== 'mirror') {
        ...
        mqtt.Client.publish(link.channel, msg, {qos: 1});
...
function slackXfer(usr, file, token, channel) {
  ...
  request(options)
    ...
    .on('finish', function() {
      ...
      mqttClient.publish(channel, msg, {qos: 1});
  });
}
